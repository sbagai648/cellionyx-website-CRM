...
class CellionyxPortal {
    ...
    async handleLogin() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.user = data.user;
                localStorage.setItem('equstech_token', this.token);
                localStorage.setItem('equstech_user', JSON.stringify(this.user));
    ...
    async loadProspects() {
        ...
        try {
            ...
            let url = '/reps/my-prospects?limit=50';
            ...
            this.prospects = Array.isArray(response) ? response : (response.prospects || []);
            console.log('Prospects to render:', this.prospects.length, this.prospects);
    ...
    renderProspects() {
        ...
        console.log('Rendering prospects:', this.prospects.length);
        const rows = this.prospects.map(prospect => {
            ...
            const prospectId = prospect.prospect_id || prospect.id || prospect.prospectId;
            
            console.log('Prospect ID:', prospectId, 'Name:', prospect.first_name, prospect.last_name);
            
            return `
                <tr data-prospect-id="${prospectId}" style="cursor: pointer;">
                    <td><strong>${prospect.first_name} ${prospect.last_name}</strong></td>
                    <td>${prospect.email}</td>
                    <td>${prospect.phone || '-'}</td>
                    <td>${prospect.organization || '-'}</td>
                    <td>${prospect.function_role || '-'}</td>
                    <td>${prospect.discipline || '-'}</td>
                    <td><span class="status-badge status-${statusClass}">${prospect.status}</span></td>
                    <td><span class="priority-badge priority-${priorityClass}">${prospect.priority}</span></td>
                    <td>${createdDate}</td>
                </tr>
            `;
        }).join('');
        ...
        console.log('Prospects table rendered with', this.prospects.length, 'rows');
    }
    
    // New methods for prospect detail panel
    showProspectDetail(prospectId, row) {
        console.log('showProspectDetail called with ID:', prospectId);
        const prospect = this.prospects.find(p => 
        ...
        if (!prospect) {
            console.error('Prospect not found with ID:', prospectId);
            console.log('Available prospects:', this.prospects.map(p => ({
        ...
        if (panel) {
            console.log('Showing prospect detail panel');
            panel.classList.add('active');
            panel.style.right = '0'; // Force it to show
        } else {
            console.error('Prospect detail panel not found in DOM!');
            alert('Detail panel not found. Please refresh the page.');
        }
    ...
    closeModal(modalId) {
        ...
        if (modal) {
            ...
            // Clear form if it's the activity modal
    ...
    async loadUsers() {
        ...
        try {
            ...
            try {
                ...
                console.log('Using mock user data while backend deploys...');
                users = [
                    { id: '1', firstName: 'Admin', lastName: 'User', email: 'admin@equstech.com', role: 'Admin', isActive: true, lastLogin: new Date() },
                    { id: '2', firstName: 'John', lastName: 'Smith', email: 'john@equstech.com', role: 'Sales Rep', isActive: true, lastLogin: new Date(Date.now() - 86400000) },
                    { id: '3', firstName: 'Jane', lastName: 'Doe', email: 'jane@equstech.com', role: 'Customer', isActive: true, lastLogin: new Date(Date.now() - 172800000) },
                    { id: '4', firstName: 'Mike', lastName: 'Johnson', email: 'mike@equstech.com', role: 'Sales Rep', isActive: false, lastLogin: new Date(Date.now() - 604800000) },
                    { id: '5', firstName: 'Sarah', lastName: 'Williams', email: 'sarah@equstech.com', role: 'Customer', isActive: true, lastLogin: null }
                ];
            }
            ...
            users.forEach(user => {
                ...
                html += `
                    <tr>
                        ...
                        <td>${lastLogin}</td>
                        <td>
                            <button class="btn btn-sm" onclick="portal.editUser('${user.id}')">Edit</button>
                            <button class="btn btn-sm btn-danger" onclick="portal.toggleUserStatus('${user.id}', ${user.isActive})">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add global click handler for prospect rows
    document.addEventListener('click', (e) => {
        const row = e.target.closest('tr[data-prospect-id]');
        if (row) {
            const prospectId = row.getAttribute('data-prospect-id');
            console.log('Global click handler: Row clicked with ID:', prospectId);
            portal.showProspectDetail(prospectId, row);
        }
    });
});
