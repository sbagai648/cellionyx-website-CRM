        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const formData = {
                    ...
                    userTypeLabel: userTypeSelect?.options[userTypeSelect?.selectedIndex]?.text || '', // Capture label
                    organization: form.querySelector('#organization')?.value?.trim() || '',
                    areaOfInterest: areaOfInterestSelect?.value || '',
                    areaOfInterestLabel: areaOfInterestSelect?.options[areaOfInterestSelect?.selectedIndex]?.text || '', // Capture label
                    intendedUse: intendedUseSelect?.value || '',
                    intendedUseLabel: intendedUseSelect?.options[intendedUseSelect?.selectedIndex]?.text || '', // Capture label
                    additionalInfo: form.querySelector('#additionalInfo')?.value?.trim() || '',
                    notes: form.querySelector('#notes')?.value?.trim() || '', // Add notes field
                    functionRole: userTypeSelect?.value || '', // Keep for backend compatibility
                    functionLabel: userTypeSelect?.options[userTypeSelect?.selectedIndex]?.text || '', // For backend
                    discipline: areaOfInterestSelect?.value || '', // Keep for backend compatibility
                    disciplineLabel: areaOfInterestSelect?.options[areaOfInterestSelect?.selectedIndex]?.text || '', // For backend
