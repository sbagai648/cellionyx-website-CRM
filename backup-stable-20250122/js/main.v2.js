...
function initCTAModal() {
    ...
    // Add click handlers to all CTA buttons (use Set to avoid duplicates)
    const ctaButtons1 = document.querySelectorAll('.cta-button');
    const ctaButtons2 = document.querySelectorAll('.btn-primary');
    const ctaButtons3 = document.querySelectorAll('[data-action="demo"]');
    
    console.log('Selector .cta-button found:', ctaButtons1.length);
    console.log('Selector .btn-primary found:', ctaButtons2.length);
    console.log('Selector [data-action="demo"] found:', ctaButtons3.length);
    
    const allCTAButtons = Array.from(new Set([
        ...ctaButtons1,
        ...ctaButtons2,
        ...ctaButtons3
    ]));
    
    console.log(`Found ${allCTAButtons.length} unique CTA buttons after deduplication`);
    
    // Process each button ONCE
    allCTAButtons.forEach((button, index) => {
        console.log(`Processing button ${index}:`, button.textContent, '| type:', button.type);
        
        // Skip submit buttons ONLY if they're inside the CTA form (not CTA trigger buttons)
        if (button.type === 'submit' && button.closest('#ctaForm')) {
        ...
        if (buttonText.includes('demo') || buttonText.includes('request')) {
            ...
            // Mark this button as handled to prevent fallback handler
            button.setAttribute('data-cta-handled', 'true');
    ...
    // Global fallback ONLY for dynamically added buttons that weren't handled above
    document.addEventListener('click', function(e) {
        ...
        if (target) {
            ...
            // Skip submit buttons only if they're in the form
            if (target.type === 'submit' && target.closest('#ctaForm')) {
                return;
            }
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Get dropdown elements to capture both value and label
                ...
                const formData = {
                    ...
                    phone: (function() {
                        ...
                        if (phoneInput && phoneInput.iti) {
                            return phoneInput.iti.getNumber() || phoneInput.value?.trim() || '';
                        }
                    ...
                    functionRole: userTypeSelect?.value || '',
                    functionLabel: userTypeSelect?.options[userTypeSelect?.selectedIndex]?.text || '',
                    discipline: areaOfInterestSelect?.value || '',
                    disciplineLabel: areaOfInterestSelect?.options[areaOfInterestSelect?.selectedIndex]?.text || '',
