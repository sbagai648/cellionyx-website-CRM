...
function initCTAModal() {
    ...
    // Handle form submission with multiple attachment methods
    console.log('🔧 Setting up form submission handlers...');
    
    function attachFormHandler() {
        const form = document.getElementById('ctaForm');
        console.log('🔍 Looking for form:', form ? 'FOUND' : 'NOT FOUND');
        ...
        if (form) {
            ...
            e.preventDefault();
            e.stopPropagation();
            console.log('🚀 FORM SUBMISSION STARTED - DEBUG v3');
            console.log('📝 Form element:', newForm);
            console.log('📝 Event:', e);
            
            // Log all form values to debug
            console.log('Form field values:', {
                firstName: newForm.querySelector('#firstName')?.value,
                lastName: newForm.querySelector('#lastName')?.value,
                email: newForm.querySelector('#email')?.value,
                userType: newForm.querySelector('#userType')?.value,
                areaOfInterest: newForm.querySelector('#areaOfInterest')?.value,
                intendedUse: newForm.querySelector('#intendedUse')?.value
            });
            ...
            try {
                ...
                const formData = {
                    firstName: newForm.querySelector('#firstName')?.value?.trim() || '',
                    lastName: newForm.querySelector('#lastName')?.value?.trim() || '',
                    email: newForm.querySelector('#email')?.value?.trim() || '',
                    phone: newForm.querySelector('#phone')?.value?.trim() || '',
                    country: newForm.querySelector('#country')?.value || '',
                    state: newForm.querySelector('#state')?.value?.trim() || '',
                    city: newForm.querySelector('#city')?.value?.trim() || '',
                    userType: newForm.querySelector('#userType')?.value || '',
                    organization: newForm.querySelector('#organization')?.value?.trim() || '',
                    areaOfInterest: newForm.querySelector('#areaOfInterest')?.value || '',
                    intendedUse: newForm.querySelector('#intendedUse')?.value || '',
                    additionalInfo: newForm.querySelector('#additionalInfo')?.value?.trim() || '',
                    functionRole: newForm.querySelector('#userType')?.value || '', // Keep for backend compatibility
                    discipline: newForm.querySelector('#areaOfInterest')?.value || '', // Keep for backend compatibility
                    ctaType: currentCTAType || 'demo',
                    page: window.location.pathname.split('/').pop() || 'index.html',
                    consent: newForm.querySelector('#consent')?.checked || false
                };
                ...
                if (!formData.userType) missingFields.push('User Type');
                ...
                if (missingFields.length > 0) {
                    ...
                    return; // Don't throw, just return to keep form intact
                }
                ...
                if (response.ok || responseData.success) {
                    ...
                    // Close modal properly (restores body overflow)
                    closeCTAModal();
    ...
    // Also attach when modal opens
    const originalOpenModal = window.openCTAModal;
    window.openCTAModal = function(type) {
        if (originalOpenModal) originalOpenModal(type);
        setTimeout(attachFormHandler, 100); // Small delay to ensure modal is rendered
    };
}
...
function openCTAModal(type = 'demo') {
    console.log('🎭 openCTAModal called with type:', type);
    
    const modal = document.getElementById('ctaModal');
    console.log('🎭 Modal element:', modal ? 'Found' : 'NOT FOUND');
    
    if (!modal) {
        console.error('❌ Modal not found - ctaModal element missing from DOM');
    ...
    console.log('🎭 Subtitle element:', subtitle ? 'Found' : 'NOT FOUND');

    // Update modal content based on type
    try {
        if (type === 'waitlist') {
            if (title) title.textContent = 'Join the Waiting List';
            if (subtitle) subtitle.textContent = 'Be among the first to experience Cellionyx';
        } else {
            if (title) title.textContent = 'Request a Demo';
            if (subtitle) subtitle.textContent = 'See Cellionyx technology in action';
        }
