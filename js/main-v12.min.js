...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        initEscapeKeyHandler(); // Add ESC key handler for modal
...
function initCTAModal() {
    ...
    allPrimaryButtons.forEach(button => {
        ...
        if (buttonText.includes('demo') || buttonText.includes('request')) {
            ...
            button.addEventListener('click', function(e) {
                ...
                // Set a flag to prevent the fallback handler from running
                button.setAttribute('data-event-handled', 'true');

                currentCTAType = 'demo';
                console.log('PRIMARY handler opening modal for:', currentCTAType);
                openCTAModal(currentCTAType);

                // Clean up the flag after a short delay
                setTimeout(() => button.removeAttribute('data-event-handled'), 500);
            });
        }
    });
    ...
    document.addEventListener('click', function(e) {
        // Don't interfere with intl-tel-input dropdown
        if (e.target.closest('.iti__flag-container, .iti__country-list')) {
            return; // Let the phone dropdown work normally
        }
        
        const target = e.target.closest('.btn-primary, .cta-btn, .nav-btn');
        if (target) {
            // CRITICAL FIX: If the specific handler already ran, do nothing.
            if (target.hasAttribute('data-event-handled')) {
                return;
            }
            ...
            if (buttonText.includes('demo') || buttonText.includes('waitlist') || 
                ...
                if (!target.hasAttribute('data-cta-handled')) {
                    e.preventDefault();
                    // Don't stop propagation if clicking on phone field elements
                    if (!e.target.closest('.iti, .iti__flag-container, .iti__country-list')) {
                        e.stopPropagation();
                    }
    ...
    window.addEventListener('click', function(e) {
        // If the click is on the modal backdrop AND not on the phone dropdown, close it
        if (e.target === modal && !e.target.closest('.iti__dropdown, .iti__flag-container')) {
            closeCTAModal();
        }
    });
    ...
    function attachFormHandler() {
        ...
        if (form) {
            ...
            if (submitButton && !submitButton._clickHandler) {
                submitButton._clickHandler = function(e) {
                ...
                submitButton.addEventListener('click', submitButton._clickHandler);
                console.log('Also attached click handler to submit button');
            }
            
            // Create the handler function
            const submitHandler = async function(e) {
            e.preventDefault();
            // Don't use stopPropagation for form submission
            ...
            try {
                ...
                if (missingFields.length > 0) {
                    ...
                    showCustomErrorMessage('Please fill in all required fields: ' + missingFields.join(', '));
                ...
                if (response.ok || responseData.success) {
                    ...
                    // Show success message - removed duplicate, using only showCustomSuccessMessage
                    const successMsg = responseData.message || 'Thank you! We\'ll be in touch shortly.';
...
function openCTAModal(type = 'demo') {
    ...
    // Call the reset function reliably every time the modal is opened.
    if (window.resetPhoneField) {
        window.resetPhoneField();
    }
...
// Initialize ESC key handler for modal
function initEscapeKeyHandler() {
    ...
    console.log('ESC key handler initialized');
}
